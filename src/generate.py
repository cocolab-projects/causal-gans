...
def generate_vec():
	...
	cf_world = generate_cf_vec(world)
...
def reformat(world):
	if(world["causal"]):
		...
		nums = [pos1, "", "", pos4]
		utt = pos1 + ((" causes " + pos4) if pos4 else "")
	else:
		return [O if world["O"] else "", "", "", E if world["bE"] else ""]
		pos1 = str(O) if world["O"] else ""
		pos4 = str(E) if world["bE"] else ""

		nums = [pos1, "", "", pos4]
		utt = pos1 + (("and " + pos4) if pos4 else "")
	return nums, utt
